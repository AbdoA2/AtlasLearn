...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    scores, cache2 = affine_forward(f1, W2, b2)
    ...
    # Compute the loss
    num_train = X.shape[0]
    shifted_scores = scores - np.max(scores, axis=1)
    exp_scores = np.exp(shifted_scores)
    props = exp_scores / np.sum(exp_scores, axis=1)
    loss = - np.mean(np.log(props[np.arange(X.shape[0]), y.T]))
    ...
    z[np.arange(X.shape[0]), y.T] = 1
    dscores = (props - z) / num_train
